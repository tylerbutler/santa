...
download-ccl-tests version="v0.6.0" force="false":
    ...
    TEST_DATA_DIR="crates/sickle/tests/test_data"
    VERSION_FILE="$TEST_DATA_DIR/.version"
    ...
    if [ "$FORCE" != "true" ] && [ -f "$VERSION_FILE" ]; then
        ...
        if [ "$CURRENT_VERSION" = "$TARGET_VERSION" ]; then
            echo "✅ CCL test data already at $TARGET_VERSION (use force=true to re-download)"
            exit 0
        fi
    ...
    if [ -z "$ZIP_URL" ] || [ "$ZIP_URL" = "null" ]; then
        echo "❌ No generated test data asset found for $TARGET_VERSION"
        exit 1
    fi
    ...
    find "$TEMP_DIR" -name "*.json" -exec cp {} "$TEST_DATA_DIR/" \;

    # Record the downloaded version
    echo "$TARGET_VERSION" > "$VERSION_FILE"

    echo "✅ CCL test data $TARGET_VERSION downloaded to $TEST_DATA_DIR/"
