...
async fn test_successful_package_installation() {
    let mock_source = MockPackageSource::new("mock-apt")
        .with_packages(vec!["existing-pkg".to_string()]);
...
async fn test_package_listing() {
    ...
    let mock_source = MockPackageSource::new("mock-brew")
        .with_packages(expected_packages.clone());
...
async fn test_batch_install_support() {
    ...
    let result = mock_source.install_packages(&["pkg1", "pkg2", "pkg3"]).await;
