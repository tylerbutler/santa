...
mod cli_parsing_tests {
    ...
    fn test_global_flags_with_subcommands() {
        ...
        let cli =
            Cli::try_parse_from(vec!["santa", "-v", "--builtin-only", "status", "--all"]).unwrap();
...
mod config_loading_tests {
    ...
    fn test_builtin_config_loading() {
        ...
        assert!(
            ...
            "Default config should have packages"
        );
    }
    ...
    fn test_load_config_from_file(temp_config: NamedTempFile) {
        ...
        assert!(
            !config.sources.is_empty(),
            "Loaded config should have sources"
        );
        assert!(
            !config.packages.is_empty(),
            "Loaded config should have packages"
        );
    }
    ...
    fn test_load_config_invalid_yaml(temp_dir: TempDir) {
        ...
        fs::write(&invalid_config, "invalid: yaml: content: [}")
...
mod integration_tests {
    ...
    fn test_build_cli_configuration() {
        ...
        assert!(
            ...
            "Should require subcommand"
        );
        assert!(
            cmd.is_arg_required_else_help_set(),
            "Should show help if no args"
        );
        ...
        assert!(
            ...
            "Should have descriptive about text"
        );
    }
