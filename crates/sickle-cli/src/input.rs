...
impl InputSource {
    pub(crate) fn from_arg(file: Option<&Path>) -> Self {
        match file {
    ...
    pub(crate) fn read(&self) -> Result<Input> {
        match self {
...
pub(crate) fn detect_format(file: Option<&Path>, explicit: Option<Format>) -> Result<Format> {
