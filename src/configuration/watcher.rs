...
impl ConfigWatcher {
    ...
    pub async fn start_watching(&mut self, data: Arc<SantaData>) -> Result<()> {
        if !self.config_path.exists() {
            info!("Config file does not exist, creating parent directory if needed: {}", 
        ...
        tokio::spawn(async move {
            info!("Started configuration file watcher for: {}", watch_path.display());
                    ...
                    match Self::reload_config(&watch_path, &data).await {
                        Ok(new_config) => {
                            ...
                            *current_config.write().await = new_config.clone();
                            ...
                            info!("Configuration reloaded successfully from: {}", watch_path.display());
                        }
                        Err(e) => {
    ...
    async fn reload_config(config_path: &Path, data: &SantaData) -> Result<SantaConfig> {
        ...
        new_config.validate_with_data(data)
            .with_context(|| "New configuration validation failed")?;

        debug!("Configuration validation passed for: {}", config_path.display());
        Ok(new_config)
    }
    ...
    pub async fn update_config(&self, new_config: SantaConfig) -> Result<()> {
        *self.current_config.write().await = new_config.clone();
        ...
        self.change_sender.send(change_event)
...
mod tests {
    ...
    async fn test_is_config_change_event() {
        ...
        assert!(ConfigWatcher::is_config_change_event(&modify_event, config_path));
        ...
        assert!(ConfigWatcher::is_config_change_event(&create_event, config_path));
        ...
        assert!(!ConfigWatcher::is_config_change_event(&other_event, config_path));
    }
...
packages: ["git"]
        ...
        assert!(error_msg.contains("Failed to load config from") || 
