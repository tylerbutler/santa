...
mod tests {
    ...
    fn test_load_real_packages_file() {
        ...
        if packages_path.exists() {
            ...
            assert!(result.is_ok(), "Should load packages successfully: {:?}", result);
            ...
            if let Some(bat) = packages.get("bat") {
                assert!(bat.is_available_in("brew"), "bat should be available in brew");
            }
            
            println!("Successfully loaded {} packages from schema", packages.len());
        } else {
    ...
    fn test_load_real_sources_file() {
        ...
        if sources_path.exists() {
            ...
            assert!(result.is_ok(), "Should load sources successfully: {:?}", result);
            ...
            if let Some(brew) = sources.get("brew") {
                ...
                assert!(brew.install.contains("{package}"), "install command should have placeholder");
            }
    ...
    fn test_load_real_config_file() {
        ...
        if config_path.exists() {
            ...
            assert!(result.is_ok(), "Should load config successfully: {:?}", result);
            
            let config = result.unwrap();
            assert!(!config.sources.is_empty(), "Should have configured sources");
            assert!(!config.packages.is_empty(), "Should have configured packages");
            
            println!("Successfully loaded config with {} sources and {} packages", 
    ...
    fn test_schema_conversion_round_trip() {
        ...
        if packages_path.exists() && sources_path.exists() {
            ...
            assert!(!legacy_packages.is_empty(), "Legacy packages should not be empty");
            assert!(!legacy_sources.is_empty(), "Legacy sources should not be empty");
            
            println!("Successfully converted {} packages and {} sources to legacy format", 
                     legacy_packages.len(), legacy_sources.len());
        } else {
