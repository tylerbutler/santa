...
pub struct SantaData {
    pub packages: HashMap<String, Vec<(KnownElves, Option<PackageData>)>>,
    // pub elf_settings: HashMap<KnownElves, PackageData>,
}

impl SantaData {
    pub fn default() -> Self {
        let brew_scoop_pacman = vec![
            (KnownElves::Brew, None),
            (KnownElves::Scoop, None),
            (KnownElves::Pacman, None),
        ];
        let apt_brew_scoop_pacman: Vec<(KnownElves, Option<PackageData>)> = vec![
            (KnownElves::Apt, None),
            (KnownElves::Scoop, None),
            (KnownElves::Pacman, None),
            (KnownElves::Brew, None),
        ];
        let mut pkgs: HashMap<String, Vec<(KnownElves, Option<PackageData>)>> = HashMap::new();
        let mut settings: HashMap<KnownElves, PackageData> = HashMap::new();
        pkgs.insert("bat".to_string(), brew_scoop_pacman.to_vec());
        pkgs.insert("bottom".to_string(), brew_scoop_pacman.to_vec());
        pkgs.insert("chezmoi".to_string(), brew_scoop_pacman.to_vec());
        pkgs.insert("direnv".to_string(), brew_scoop_pacman.to_vec());
        pkgs.insert("dust".to_string(), brew_scoop_pacman.to_vec());
        pkgs.insert("exa".to_string(), brew_scoop_pacman.to_vec());
        pkgs.insert("fd".to_string(), brew_scoop_pacman.to_vec());
        pkgs.insert(
            "fnm".to_string(),
            vec![(KnownElves::Brew, None), (KnownElves::Scoop, None)],
        );
        pkgs.insert("fzf".to_string(), brew_scoop_pacman.to_vec());
        pkgs.insert(
        ...
        pkgs.insert(
            "ghq".to_string(),
            vec![(KnownElves::Scoop, None), (KnownElves::Brew, None)],
        );
        pkgs.insert(
            "git-delta".to_string(),
            vec![
        ...
        pkgs.insert("tre-command".to_string(), vec![(KnownElves::Brew, None), (KnownElves::Scoop, None)]);
